...
/* ===== GKM DEDUPE AND RATING FIX V3 ===== */
(function () {
  if (window.__gkmDedupeRatingFixV3) return;
  window.__gkmDedupeRatingFixV3 = true;
  ...
  function cardTitle(card) {
    ...
    // fallback: берём первую жирную/крупную строку из текста
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (/^(фильм|сериал|аниме|мультфильм|топ|новинка)$/i.test(line)) continue;
      if (/^\d{4}\s*·/.test(line)) continue;
      if (/класс\s*·/i.test(line)) continue;
  ...
  function cardType(card) {
    var text = String(card.innerText || card.textContent || "");

    if (text.includes("Аниме")) return "anime";
    if (text.includes("Мультфильм")) return "cartoon";
    if (text.includes("Сериал")) return "tv";
    if (text.includes("Фильм")) return "movie";
  ...
  function cardPoster(card) {
    ...
    // режем query/cache, чтобы один и тот же постер сравнивался нормально
    src = src.split("?")[0];
  ...
  function yearFromCard(card) {
    var text = String(card.innerText || card.textContent || "");
    var m = text.match(/\b(19\d{2}|20\d{2})\b/);
    return m ? m[1] : "";
  }

  function dedupeCardsDom() {
    var cards = Array.from(document.querySelectorAll(".card"));
    if (!cards.length) return 0;

    var seenStrong = new Set();
    var seenTitle = new Map();
    var removed = 0;

    cards.forEach(function (card) {
      var title = norm(cardTitle(card));
      if (!title) return;

      var type = cardType(card);
      var poster = cardPoster(card);
      var year = yearFromCard(card);

      // 1. самый сильный ключ: тип + название + постер
      var strongKey = type + "|" + title + "|" + poster;

      // 2. мягкий ключ: тип + название, без года
      // Именно он убирает "Бойцовский клуб 1999/2000", "Побег 1994/2019", Форресты и т.д.
      var titleKey = type + "|" + title;

      if (poster && seenStrong.has(strongKey)) {
        card.remove();
        removed++;
        return;
      }
      ...
      if (seenTitle.has(titleKey)) {
        var old = seenTitle.get(titleKey);

        // Если название одинаковое, это почти всегда дубль в твоей базе.
        // Оставляем первый попавшийся в выдаче, второй удаляем.
        card.remove();
        removed++;
        return;
      }

      if (poster) seenStrong.add(strongKey);
      seenTitle.set(titleKey, { card: card, year: year, poster: poster });
    });

    return removed;
  }

  function fixFakeRatingBadges() {
    var nodes = Array.from(document.querySelectorAll(".card *"));

    nodes.forEach(function (el) {
      var txt = String(el.textContent || "").trim();

      if (!/S-класс\s*·\s*10\.0/i.test(txt)) return;

      // Не даём левым фильмам с единичными голосами выглядеть как абсолютный топ.
      // Лучше пусть будет честно "Новый".
      el.textContent = "Новый · мало оценок";
      el.title = "Рейтинг 10.0 скрыт: мало оценок";
    });
  }
  ...
  var debouncedCleanup = debounce(cleanup, 180);
  ...
  function start() {
    ...
    // На всякий случай, потому что карточки дорисовываются пачками
  ...
  window.GKM_APP_CLEAN_VERSION = "performance-v3-hard-dedupe-rating-fix-2026-06-12";
  console.log("GKM DEDUPE AND RATING FIX V3 установлен");
})();
