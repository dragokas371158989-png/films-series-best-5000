const DATA_URL = "movies_updates.json";
const PAGE_SIZE = 40;
...
const favKey = "filmsSeriesFav";
const historyKey = "filmsSeriesHistory";
...
async function loadData() {
  $("statusText").textContent = "Загрузка movies_updates.json...";
  const res = await fetch(DATA_URL + "?v=" + Date.now(), { cache: "no-store" });
  if (!res.ok) throw new Error("Не удалось загрузить movies_updates.json: " + res.status);
...
function fillFilters() {
  const years = [...new Set(allMovies.map(getYear).filter(Boolean))].sort((a,b) => Number(b)-Number(a));
  const genres = [...new Set(allMovies.flatMap(getGenres))].sort((a,b) => a.localeCompare(b, "ru"));
...
function applyFilters() {
  ...
  if (q) {
    list = list.filter(m => {
      const hay = normalize([
        m.ru, m.en, m.year, m.type, m.status, m.overview,
        ...getGenres(m)
      ].join(" "));
...
function openDetails(m) {
  ...
  $("detailMeta").textContent = `${m.year || "—"} · ${m.type || "—"} · рейтинг ${getRating(m).toFixed(1)} · голосов ${m.votes || 0}`;
...
function setupEvents() {
  ...
  $("themeBtn").addEventListener("click", () => {
    ...
    localStorage.setItem("filmsTheme", document.body.classList.contains("light") ? "light" : "dark");
  });

  if (localStorage.getItem("filmsTheme") === "light") {
...
function showError(e) {
  ...
  $("grid").innerHTML = `<div class="card"><div class="card-body">Не удалось загрузить базу. Проверь, что рядом лежит movies_updates.json.</div></div>`;
}
