...
async function renderHome() {
  // GKM V250: убираем сломанную витрину Популярное/Топ/Новинки.
  // Главная снова показывает обычную сетку карточек, как нормальный каталог.
  currentMode = "page";
  currentTab = "all";
  currentPage = 1;
  currentPages = 1;
  setActiveTab("all");
  setStatus("Открываю главную...");
  await loadFastPage("all", 1);
}
...
function makeSearchWorker() {
  ...
  const absoluteSearchLiteUrl = new URL(`${SEARCH_LITE_URL}?v=250`, window.location.href).href;
  const absoluteSearchFullUrl = new URL(`${SEARCH_URL}?v=250`, window.location.href).href;
  ...
  const code = `
    ...
    async function loadShard(key){if(!key)return [];if(!shardPromises.has(key)){const url=SHARD_BASE+encodeURIComponent(key)+".json?v=250";shardPromises.set(key,fetch(url,{cache:"force-cache"}).then(r=>{if(r.status===404)return [];if(!r.ok)return [];return r.json();}).catch(()=>[]));}return shardPromises.get(key);}
...
/* GKM V250 NORMAL HOME GRID RESTORE - home dashboard disabled */
