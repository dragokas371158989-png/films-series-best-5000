...
function addAnimeTvApkDownloadBlock() {
  if (document.getElementById("animeTvApkDownloadBlock")) return;

  const block = document.createElement("section");
  block.id = "animeTvApkDownloadBlock";
  block.className = "anime-tv-download-block";

  block.innerHTML = `
    <div class="anime-tv-download-info">
      <h2>ГОЛУБЬ Anime TV</h2>
      <p>Версия для телефона и Android TV. Установи APK и открывай Anime TV как приложение.</p>
    </div>

    <a class="anime-tv-apk-btn" href="downloads/golub-anime-tv.apk.apk" download>
      📥 Скачать Anime TV APK
    </a>
  `;

  const css = document.createElement("style");
  css.id = "animeTvApkDownloadStyle";

  css.textContent = `
    .anime-tv-download-block {
      ...
      justify-content: space-between;
      gap: 18px;
      background: linear-gradient(135deg, rgba(124, 58, 237, 0.42), rgba(0, 212, 255, 0.16));
      border: 1px solid rgba(0, 229, 255, 0.45);
      box-shadow: 0 0 24px rgba(0, 229, 255, 0.18);
    }

    .anime-tv-download-info h2 {
      margin: 0 0 8px 0;
      color: #ffffff;
      font-size: 26px;
      font-weight: 900;
    ...
    .anime-tv-apk-btn {
      ...
      font-weight: 900;
      font-size: 16px;
      background: linear-gradient(135deg, #ff006e, #7c3aed, #00d4ff);
      border: 1px solid rgba(0, 229, 255, 0.65);
      box-shadow: 0 0 24px rgba(255, 0, 110, 0.38);
    }

    .anime-tv-apk-btn:hover {
  ...
  if (!document.getElementById("animeTvApkDownloadStyle")) {
  ...
  if (homeHero && homeHero.parentNode) {
    homeHero.insertAdjacentElement("afterend", block);
    return;
  }

  if (seoBlock && seoBlock.parentNode) {
    seoBlock.insertAdjacentElement("afterend", block);
    return;
  }
...
document.addEventListener("DOMContentLoaded", addAnimeTvApkDownloadBlock);
setTimeout(addAnimeTvApkDownloadBlock, 500);
setTimeout(addAnimeTvApkDownloadBlock, 1500);
setTimeout(addAnimeTvApkDownloadBlock, 3000);
