...
function makeSearchWorker() {
  ...
  const absoluteSearchLiteUrl = new URL(`${SEARCH_LITE_URL}?v=117`, window.location.href).href;
  const absoluteSearchFullUrl = new URL(`${SEARCH_URL}?v=117`, window.location.href).href;
  ...
  const code = `
    ...
    function animeTopScore(x){const r=rating(x);const v=Math.max(1,votes(x));const target=9000000;const voteCloseness=Math.min(v,target)/target;const votePower=Math.log10(v);return r*1000000 + voteCloseness*450000 + Math.min(v,3000000)/15 + votePower*50000 + poster(x)*10000;}
    function applyAnimeTopDedupe(){const target=9000000;const thresholds=[9000000,7000000,5000000,3000000,2000000,1500000,1000000,700000,500000,300000,100000];let source=rows;for(const th of thresholds){const part=rows.filter(r=>votes(r.item)>=th);if(part.length>=100){source=part;break;}if(th===100000)source=part.length?part:rows;}const cap=new Map();const out=[];for(const row of source){const key=franchiseKey(row.item);const count=cap.get(key)||0;if(count>=2)continue;cap.set(key,count+1);out.push(row);if(out.length>=100)break;}if(out.length<100){for(const row of rows){if(out.includes(row))continue;const key=franchiseKey(row.item);const count=cap.get(key)||0;if(count>=2)continue;cap.set(key,count+1);out.push(row);if(out.length>=100)break;}}rows=out;self.__animeTopThreshold=source.length?Math.min(...source.map(r=>votes(r.item))):0;}
    function sortRows(sort, hasQuery, tab){const pr=(a,b)=>poster(b.item)-poster(a.item);if(tab==="anime_top"){rows.sort((a,b)=>pr(a,b)||animeTopScore(b.item)-animeTopScore(a.item)||votes(b.item)-votes(a.item)||rating(b.item)-rating(a.item));applyAnimeTopDedupe();}else if(sort==="rating")rows.sort((a,b)=>pr(a,b)||rating(b.item)-rating(a.item)||votes(b.item)-votes(a.item));else if(sort==="votes")rows.sort((a,b)=>pr(a,b)||votes(b.item)-votes(a.item)||rating(b.item)-rating(a.item));else if(sort==="year")rows.sort((a,b)=>pr(a,b)||Number(year(b.item)||0)-Number(year(a.item)||0)||votes(b.item)-votes(a.item));else if(sort==="year_old")rows.sort((a,b)=>pr(a,b)||Number(year(a.item)||9999)-Number(year(b.item)||9999)||votes(b.item)-votes(a.item));else if(sort==="title")rows.sort((a,b)=>pr(a,b)||title(a.item).localeCompare(title(b.item),"ru"));else if(hasQuery)rows.sort((a,b)=>pr(a,b)||b.score-a.score);else rows.sort((a,b)=>pr(a,b)||(rating(b.item)*100000+Math.min(votes(b.item),250000)+Number(year(b.item)||0))-(rating(a.item)*100000+Math.min(votes(a.item),250000)+Number(year(a.item)||0)));}
    ...
    async function loadShard(key){if(!key)return [];if(!shardPromises.has(key)){const url=SHARD_BASE+encodeURIComponent(key)+".json?v=117";shardPromises.set(key,fetch(url,{cache:"force-cache"}).then(r=>{if(r.status===404)return [];if(!r.ok)return [];return r.json();}).catch(()=>[]));}return shardPromises.get(key);}
