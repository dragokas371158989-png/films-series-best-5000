...
/* GKM V187 VISIBLE FILTER COUNTER START */
(function () {
  "use strict";

  window.GKM_V187_VISIBLE_FILTER_COUNTER_VERSION = "v187-visible-filter-counter-2026-06-24";

  /*
    V187:
    V186 кнопки срабатывали, но на нормальной странице визуально могло казаться,
    что ничего не произошло. Теперь:
    - каждая кнопка показывает счётчик: "Осталось X из Y";
    - "Популярное" не просто фильтр, а сортировка по голосам/рейтингу;
    - "С постером", "7+", "от 1000", "Скрыть мусор" реально скрывают через класс;
    - если скрывать нечего, пишет "ничего не скрыто";
    - "Сброс качества" возвращает все карточки и исходную прозрачность;
    - работает без вмешательства в загрузку базы.
  */

  const LS = {
    quick: "GKM_V187_QUICK_FILTER",
    state: "GKM_V187_STATE",
    history: "GKM_V187_HISTORY",
    folders: "GKM_V187_FOLDERS"
  };

  const FOLDERS = ["Смотреть позже", "Любимое", "Уже смотрел", "Брошено", "Для жены", "Аниме", "Фильмы на вечер"];

  function safe(name, fn) {
    try { return fn(); } catch (e) { console.warn("GKM V187:", name, e); toast("Ошибка: " + name); return null; }
  }
  ...
  function isCard(el) {
    if (!el || el.nodeType !== 1) return false;
    if (el.closest && (el.closest(".gkm-v187-quickbar") || el.closest(".gkm-v187-folder-menu") || el.closest(".gkm-v187-panel"))) return false;

    const t = norm(el.textContent);
    if (t.length < 18 || t.length > 1800) return false;
    if (!el.querySelector("img")) return false;

    const hasMeta = t.includes("★") || t.includes("фильм") || t.includes("аниме") || t.includes("сериал") || t.includes("мультфильм");
  ...
  function cards() {
    const selectors = [
      ...
      "[class*='card']", "[class*='movie']", "[class*='item']", "main div", "section div"
    ].join(",");

    const raw = Array.from(document.querySelectorAll(selectors)).filter(isCard);

    // Убираем вложенные куски карточек, оставляем самый верхний нормальный контейнер.
    const filtered = raw.filter(c => !raw.some(o => o !== c && o.contains(c) && isCard(o)));

    // Убираем повторы одного и того же DOM.
  ...
  function hasPoster(card) {
    ...
    if (!src) return false;
    if (src.includes("placeholder") || src.includes("data:image/svg")) return false;
    return true;
  }
  ...
  function trash(card) {
    ...
    if (title.includes("untitled") || title.includes("без названия")) return true;
  ...
  function qualityScore(card) {
    ...
    s += Math.min(v, 2500000) / 4000;
    s += r * 60;

    if (r >= 9.8 && v < 100) s -= 900;
    if (r >= 9.5 && v < 500) s -= 450;
    if (v > 0 && v < 50) s -= 260;
    if (y >= 2020) s += 25;
    if (title.length <= 2 || /^[a-z0-9]{1,3}$/.test(title)) s -= 650;
  ...
  function resetCards() {
    cards().forEach(c => {
      c.classList.remove("gkm-v187-hidden", "gkm-v187-trash-card", "gkm-v187-random-pick");
  ...
  function sortByQuality() {
    ...
    if (!found || !found.cards.length) return 0;
  ...
  function sortByPopular() {
    ...
    const rows = found.cards.map((card, idx) => ({
      card,
      idx,
      votes: parseVotes(card),
      rating: parseRating(card),
      score: qualityScore(card)
    }));
  ...
  function applyFilter(mode, silent) {
    const all = cards();
    if (!all.length) {
      if (!silent) toast("Карточки пока не найдены");
      updateCounter(0, 0, mode);
      return;
    }
    ...
    fresh.forEach(card => {
      ...
      if (mode === "popular") show = true;
      if (mode === "hideTrash") show = !trash(card);
      if (mode === "rating7") show = r >= 7;
      if (mode === "votes1000") show = v >= 1000;
      if (!mode) show = true;

      card.classList.toggle("gkm-v187-hidden", !show);
      card.classList.toggle("gkm-v187-trash-card", trash(card));
    ...
    document.querySelectorAll("[data-gkm-v187-filter]").forEach(b => b.classList.toggle("active", (b.dataset.gkmV187Filter || "") === (mode || "")));

    updateCounter(visible, fresh.length, mode);

    if (!silent) {
      if (!mode) toast("Сброшено: показаны все " + fresh.length);
      else if (mode === "popular") toast("Популярное отсортировано: " + fresh.length);
      else if (hidden === 0) toast("Ничего не скрыто: " + visible + " из " + fresh.length);
      else toast("Осталось " + visible + " из " + fresh.length);
    }
  }

  function updateCounter(visible, total, mode) {
    let box = document.querySelector(".gkm-v187-counter");
    if (!box) {
      box = document.createElement("div");
      box.className = "gkm-v187-counter";
      const bar = document.querySelector(".gkm-v187-quickbar");
    ...
    const name = ({
      ...
      poster: "С постером",
      popular: "Популярное",
      hideTrash: "Скрыть мусор",
      rating7: "7+",
      votes1000: "от 1000"
    })[mode] || "Без фильтра";

    box.textContent = total ? `${name}: ${visible} из ${total}` : `${name}: карточек нет`;
  }

  function smartRandom() {
    const pool = cards().filter(c => !c.classList.contains("gkm-v187-hidden") && !trash(c) && hasPoster(c) && parseVotes(c) >= 100);
    const fallback = cards().filter(c => !c.classList.contains("gkm-v187-hidden"));
    const arr = pool.length ? pool : fallback;
    if (!arr.length) { toast("Нет карточек для выбора"); return; }

    const pick = arr[Math.floor(Math.random() * arr.length)];
    pick.scrollIntoView({behavior:"smooth", block:"center"});
    pick.classList.add("gkm-v187-random-pick");
    setTimeout(() => pick.classList.remove("gkm-v187-random-pick"), 2400);
  ...
  function showHistory() {
    ...
    panel.className = "gkm-v187-panel";
    panel.innerHTML = `
      <div class="gkm-v187-box">
        <div class="gkm-v187-box-head">
          <h2>🕘 История+</h2>
          <button data-gkm-v187-action="closePanel">✕</button>
        </div>
        <div class="gkm-v187-box-actions"><button data-gkm-v187-action="clearHistory">Очистить</button></div>
        <div class="gkm-v187-history-list">
          ${h.length ? h.map(x => `
            <button data-gkm-v187-history-q="${esc(x.value)}">
  ...
  function closePanel() { document.querySelectorAll(".gkm-v187-panel").forEach(x => x.remove()); }
  ...
  function addFolderButtons() {
    cards().forEach(card => {
      if (card.querySelector(".gkm-v187-folder-btn")) return;

      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "gkm-v187-folder-btn";
      btn.textContent = "+";
  ...
  function showFolderMenu(card, btn) {
    document.querySelectorAll(".gkm-v187-folder-menu").forEach(x => x.remove());

    const title = titleOf(card);
    const d = getFolders();
    const menu = document.createElement("div");
    menu.className = "gkm-v187-folder-menu";
    menu.innerHTML = `<b>Добавить:</b>${Object.keys(d).map(f => `<button data-gkm-v187-folder="${esc(f)}">${esc(f)}</button>`).join("")}`;
    document.body.appendChild(menu);

    const r = btn.getBoundingClientRect();
    const w = 230;
    const maxH = Math.min(350, window.innerHeight - 20);
    menu.style.width = w + "px";
    menu.style.maxHeight = maxH + "px";
    menu.style.overflowY = "auto";
    menu.style.left = Math.min(window.innerWidth - w - 8, Math.max(8, r.right - w)) + "px";
    menu.style.top = Math.min(r.bottom + 8, window.innerHeight - maxH - 8) + "px";

    menu.querySelectorAll("[data-gkm-v187-folder]").forEach(x => {
      x.addEventListener("click", e => {
        e.preventDefault(); e.stopPropagation();
        const folder = x.dataset.gkmV187Folder;
  ...
  function addMiniDetails() {
    cards().forEach(card => {
      if (card.querySelector(".gkm-v187-mini")) return;

      const mini = document.createElement("div");
      mini.className = "gkm-v187-mini";
  ...
  function addQuickBar() {
    document.querySelectorAll(".gkm-v184-quickbar,.gkm-v185-quickbar,.gkm-v186-quickbar,.gkm-v187-quickbar").forEach(x => x.remove());

    const bar = document.createElement("div");
    bar.className = "gkm-v187-quickbar";
    bar.innerHTML = `
      <button data-gkm-v187-filter="normal">🔥 Только норм</button>
      <button data-gkm-v187-filter="poster">🎬 С постером</button>
      <button data-gkm-v187-filter="popular">👑 Популярное</button>
      <button data-gkm-v187-filter="hideTrash">🧹 Скрыть мусор</button>
      <button data-gkm-v187-filter="rating7">⭐ 7+</button>
      <button data-gkm-v187-filter="votes1000">🗳 от 1000</button>
      <button data-gkm-v187-filter="">Сброс качества</button>
      <button data-gkm-v187-action="random">🎲 Умное случайное</button>
      <button data-gkm-v187-action="restore">↩ Вернуть место</button>
      <button data-gkm-v187-action="history">🕘 История+</button>
      <div class="gkm-v187-counter">Без фильтра</div>
    `;
  ...
  function handleClick(e) {
    const filterBtn = e.target.closest && e.target.closest("[data-gkm-v187-filter]");
    if (filterBtn) {
      e.preventDefault(); e.stopPropagation();
      return safe("filter", () => applyFilter(filterBtn.dataset.gkmV187Filter || "", false));
    }

    const actionBtn = e.target.closest && e.target.closest("[data-gkm-v187-action]");
    if (actionBtn) {
      e.preventDefault(); e.stopPropagation();
      const a = actionBtn.dataset.gkmV187Action;
    ...
    const hist = e.target.closest && e.target.closest("[data-gkm-v187-history-q]");
    if (hist) {
      e.preventDefault(); e.stopPropagation();
      const input = getSearchInput();
      if (input) {
        input.value = hist.dataset.gkmV187HistoryQ || "";
        input.dispatchEvent(new Event("input", {bubbles:true}));
        input.dispatchEvent(new Event("change", {bubbles:true}));
      }
      closePanel();
    ...
    if (!e.target.closest || !e.target.closest(".gkm-v187-folder-menu,.gkm-v187-folder-btn")) {
      document.querySelectorAll(".gkm-v187-folder-menu").forEach(x => x.remove());
    }
  }

  function toast(msg) {
    let t = document.querySelector(".gkm-v187-toast");
    if (!t) {
      t = document.createElement("div");
      t.className = "gkm-v187-toast";
      document.body.appendChild(t);
    }
    t.textContent = msg;
    t.classList.add("show");
    clearTimeout(window.__gkmV187Toast);
    window.__gkmV187Toast = setTimeout(() => t.classList.remove("show"), 1900);
  }

  function cycle() {
    safe("buttons", addQuickBarIfMissing);
    safe("mini", addMiniDetails);
    safe("folders", addFolderButtons);
    safe("filterSilent", () => applyFilter(localStorage.getItem(LS.quick) || "", true));
  }

  function addQuickBarIfMissing() {
    if (!document.querySelector(".gkm-v187-quickbar")) addQuickBar();
  }

  function schedule() {
    clearTimeout(window.__gkmV187Cycle);
    window.__gkmV187Cycle = setTimeout(cycle, 550);
  }

  function saveStateSoon() {
    clearTimeout(window.__gkmV187Save);
    window.__gkmV187Save = setTimeout(savePlace, 300);
  }

  function addStyles() {
    if (document.querySelector("#gkm-v187-style")) return;
    const style = document.createElement("style");
    style.id = "gkm-v187-style";
    style.textContent = `
      .gkm-v187-quickbar{display:flex;flex-wrap:wrap;gap:8px;padding:10px 8px;border-top:1px solid rgba(0,216,255,.25);border-bottom:1px solid rgba(120,60,255,.25)}
      .gkm-v187-quickbar button,.gkm-v187-panel button,.gkm-v187-folder-menu button{border:1px solid #00d8ff;background:linear-gradient(135deg,#5a25d6,#04c9f4);color:#fff;border-radius:14px;padding:10px 14px;font-weight:900;cursor:pointer;box-shadow:0 0 14px rgba(0,216,255,.22)}
      .gkm-v187-quickbar button.active{background:linear-gradient(135deg,#ffae00,#b13cff)}
      .gkm-v187-counter{display:flex;align-items:center;padding:9px 13px;border:1px solid rgba(0,216,255,.55);border-radius:14px;background:rgba(0,216,255,.12);color:#dffaff;font-weight:900}
      .gkm-v187-hidden{display:none!important}
      .gkm-v187-trash-card{opacity:.62}
      .gkm-v187-mini{display:none;position:absolute;left:8px;right:8px;bottom:8px;z-index:60;padding:10px;border:1px solid rgba(0,216,255,.6);border-radius:14px;background:rgba(5,7,25,.94);color:#fff;box-shadow:0 0 20px rgba(0,216,255,.25);pointer-events:none}
      *:hover>.gkm-v187-mini{display:flex;flex-direction:column;gap:4px}
      .gkm-v187-mini b{font-size:14px}.gkm-v187-mini span{font-size:12px;color:#d7d2ff}
      .gkm-v187-folder-btn{position:absolute!important;right:9px!important;top:72px!important;z-index:65!important;width:32px!important;height:32px!important;border-radius:50%!important;border:1px solid #00d8ff!important;background:linear-gradient(135deg,#5a25d6,#04c9f4)!important;color:#fff!important;font-weight:1000!important;cursor:pointer!important}
      .gkm-v187-folder-menu{position:fixed!important;z-index:9999999!important;display:flex!important;flex-direction:column!important;gap:7px!important;padding:12px!important;border:1px solid rgba(0,216,255,.55)!important;border-radius:16px!important;background:rgba(7,8,28,.97)!important;color:#fff!important;box-shadow:0 0 24px rgba(0,216,255,.3)!important}
      .gkm-v187-folder-menu b{font-size:15px;margin-bottom:3px}.gkm-v187-folder-menu button{width:100%;font-size:14px;padding:9px 10px}
      .gkm-v187-panel{position:fixed;inset:0;z-index:9999998;overflow:auto;padding:28px;background:rgba(2,4,16,.78);backdrop-filter:blur(4px)}
      .gkm-v187-box{max-width:900px;margin:0 auto;padding:18px;border:1px solid rgba(0,216,255,.4);border-radius:18px;background:rgba(10,8,35,.96);color:#fff}
      .gkm-v187-box-head{display:flex;justify-content:space-between;align-items:center;gap:12px}.gkm-v187-history-list{display:flex;flex-direction:column;gap:8px}.gkm-v187-history-list button{display:grid;grid-template-columns:100px 1fr auto;gap:10px;align-items:center;text-align:left}.gkm-v187-history-list em{color:#cfc9ff;font-style:normal;font-size:12px}
      .gkm-v187-random-pick{outline:4px solid #00d8ff!important;box-shadow:0 0 35px rgba(0,216,255,.85)!important}
      .gkm-v187-toast{position:fixed;left:50%;bottom:26px;transform:translateX(-50%) translateY(30px);z-index:10000000;background:linear-gradient(135deg,#5a25d6,#04c9f4);color:#fff;border:1px solid #00d8ff;border-radius:14px;padding:12px 18px;font-weight:900;box-shadow:0 0 24px rgba(0,216,255,.45);opacity:0;transition:.2s}
      .gkm-v187-toast.show{opacity:1;transform:translateX(-50%) translateY(0)}
    `;
    document.head.appendChild(style);
  }
  ...
  function init() {
    try {
      localStorage.removeItem("GKM_V184_STATE");
      localStorage.removeItem("GKM_V185_QUICK_FILTER");
      localStorage.removeItem("GKM_V186_QUICK_FILTER");
    } catch(e) {}
    ...
    console.log("GKM: v187-visible-filter-counter-2026-06-24");
  }

  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", init);
  else init();

  window.GKM_V187_APPLY = cycle;
  window.GKM_V187_FILTER = applyFilter;
  window.GKM_V187_CARDS_COUNT = function(){ return cards().length; };
  window.GKM_V187_RANDOM = smartRandom;
  window.GKM_V187_HISTORY = showHistory;
  window.GKM_V187_RESTORE = restorePlace;
})();
/* GKM V187 VISIBLE FILTER COUNTER END */
