...
/* GKM V230 REAL SYNOPSIS FIX START */
(function(){
  function txt(v){ return String(v == null ? "" : v).trim(); }
  function norm(v){
  ...
  function titleOfV230(item){
    return txt(item && (item.title || item.name || item.ru || item.en || item.original_title || item.originalTitle));
  }
  function yearOfV230(item){
    const raw = txt(item && (item.year || item.release_date || item.first_air_date || item.date));
    const m = raw.match(/(19\d{2}|20\d{2})/);
    return m ? m[1] : raw;
  }
  function typeOfV230(item){
    return txt(item && (item.type || item.category || item.section || item.media_type)) || "Проект";
  }
  function genresOfV230(item){
    if (typeof getGenres === "function") {
      try { return getGenres(item); } catch(e){}
    }
    return arr(item && item.genres);
  }
  function ratingOfV230(item){
    if (typeof getRating === "function") {
      try { return getRating(item); } catch(e){}
    }
    return Number(item && (item.rating || item.vote_average) || 0);
  }
  function votesOfV230(item){
    if (typeof getVotes === "function") {
      try { return getVotes(item); } catch(e){}
    }
    return Number(item && (item.votes || item.vote_count || item.scored_by) || 0);
  }

  const REAL_SYNOPSIS = new Map(Object.entries({
    "побег из шоушенка": "Банкир Энди Дюфрейн получает пожизненный срок за убийство жены и её любовника, хотя сам настаивает на своей невиновности. В мрачной тюрьме Шоушенк он сталкивается с жестокостью охраны, тюремными порядками и полной потерей прежней жизни. Постепенно Энди находит союзника в заключённом Реде, помогает администрации с финансами и годами сохраняет главное — спокойный ум, достоинство и надежду на свободу. Это тюремная драма не столько про побег, сколько про выдержку, дружбу, несправедливость и веру в то, что человек может остаться собой даже там, где у него отняли всё.",

    "крестный отец": "История семьи Корлеоне — одной из самых влиятельных мафиозных династий Америки. Дон Вито Корлеоне держит власть не только страхом, но и личными связями, уважением и строгим кодексом чести. Когда на семью начинается давление со стороны конкурентов, его младший сын Майкл, который сначала хочет жить отдельно от криминального мира, постепенно втягивается в дела семьи. Фильм показывает, как власть, верность, насилие и семейные обязательства меняют человека и превращают наследника в нового главу мафии.",

    "темный рыцарь": "Готэм оказывается на грани хаоса, когда появляется Джокер — преступник без правил, которому важны не деньги и власть, а разрушение самой идеи порядка. Бэтмен, комиссар Гордон и прокурор Харви Дент пытаются остановить волну насилия, но Джокер заставляет каждого из них делать морально тяжёлый выбор. Это супергеройский триллер о цене справедливости, страхе, символах и тонкой границе между героем и человеком, который ради победы может потерять себя.",

    "интерстеллар": "В будущем Земля умирает: урожаи гибнут, пыльные бури становятся нормой, а человечество постепенно теряет шанс выжить. Бывший пилот Купер отправляется в космическую экспедицию через червоточину, чтобы найти новый дом для людей. На Земле остаются его дети, и особенно дочь Мёрф, связь с которой становится сердцем истории. Это научно-фантастическая драма о космосе, времени, любви, жертве и попытке спасти будущее, даже если цена — целая жизнь вдали от семьи.",

    "форрест гамп": "Форрест Гамп — простой и добрый человек, который случайно оказывается рядом с ключевыми событиями американской истории второй половины XX века. Он воюет во Вьетнаме, становится спортивной знаменитостью, бизнесменом и символом невероятной удачи, но в душе остаётся тем же искренним человеком. Главная линия его жизни — любовь к Дженни, которую он пронёс через годы. Это история о судьбе, доброте, потере, любви и том, как человек без хитрости может пройти через огромный мир и остаться настоящим.",

    "бойцовский клуб": "Безымянный герой страдает от бессонницы, пустоты и ощущения, что его жизнь превратилась в набор покупок, работы и бессмысленных правил. Встреча с харизматичным Тайлером Дёрденом приводит к созданию подпольного бойцовского клуба, где мужчины выпускают злость и чувствуют себя живыми. Но клуб быстро превращается во что-то намного опаснее. Это мрачная сатира о потреблении, кризисе личности, подавленной агрессии и разрушительной свободе, которая выходит из-под контроля.",

    "матрица": "Программист Томас Андерсон, известный как Нео, узнаёт, что привычный мир — иллюзия, созданная машинами для контроля над человечеством. Морфеус и Тринити выводят его из симуляции и показывают настоящую реальность: люди живут в руинах, а машины используют их как источник энергии. Нео должен понять, кто он на самом деле, и принять роль избранного. Это культовая фантастика о свободе выбора, природе реальности, контроле, пробуждении и борьбе человека против системы.",

    "властелин колец братство кольца": "Хоббит Фродо получает Кольцо Всевластия — древний артефакт, от которого зависит судьба Средиземья. Чтобы не дать тёмному властелину Саурону вернуть силу, Фродо должен отправиться к Роковой горе и уничтожить кольцо. В путь с ним идут люди, эльф, гном, маг и друзья-хоббиты. Это начало большого фэнтези-путешествия о дружбе, соблазне властью, страхе перед злом и маленьких героях, которым приходится нести ношу огромного мира.",

    "джокер": "Артур Флек живёт в Готэме, работает клоуном и мечтает стать комиком, но сталкивается с бедностью, унижением, болезнью и равнодушием общества. Постепенно его психика ломается, а личная боль превращается в образ Джокера — символ злости и хаоса для города, который давно готов взорваться. Это психологическая драма о человеке на краю, одиночестве, насилии, социальном давлении и рождении опасного мифа.",

    "волк с уолл стрит": "Джордан Белфорт начинает карьеру брокера и быстро понимает, что на жадности, обмане и агрессивных продажах можно построить огромные деньги. Его компания превращается в фабрику роскоши, наркотиков, вечеринок и финансовых махинаций. Чем выше он поднимается, тем сильнее рушатся моральные границы. Это энергичная криминальная сатира о жадности, власти денег, зависимости от успеха и мире, где обман продаётся как мечта.",

    "атака титанов": "Люди живут за огромными стенами, спасаясь от титанов — гигантских существ, пожирающих людей. После разрушения стены Эрен Йегер теряет дом и клянётся уничтожить титанов. Вместе с Микасой и Армином он вступает в армию, но чем дальше развивается война, тем сложнее становится правда о мире за стенами. Это мрачное аниме о свободе, мести, войне, пропаганде, цене ненависти и том, как враг может оказаться не таким простым, как казалось.",

    "наруто": "Наруто Узумаки — шумный и упрямый подросток-ниндзя, которого деревня долго отвергала из-за запечатанного внутри него Девятихвостого лиса. Он мечтает стать Хокаге, чтобы добиться признания и доказать свою ценность. Вместе с Саске, Сакурой и учителем Какаши он проходит миссии, тренировки и тяжёлые испытания. Это история о дружбе, одиночестве, силе воли, боли прошлого и желании стать человеком, которого наконец признают.",

    "ван пис": "Монки Д. Луффи отправляется в море, чтобы найти легендарное сокровище One Piece и стать Королём пиратов. По пути он собирает команду, каждый участник которой имеет свою мечту, боль и цель. Их путешествие превращается в огромную историю о свободе, дружбе, приключениях, жестоких режимах и тайнах мира. Это аниме для тех, кто любит большие вселенные, сильные эмоции, юмор и долгий путь героев к мечте.",

    "тетрадь смерти": "Старшеклассник Лайт Ягами находит тетрадь, способную убивать людей, если записать в неё имя жертвы. Решив построить новый мир без преступников, он становится загадочным убийцей Кирой. Его противником становится гениальный детектив L, который начинает опасную интеллектуальную дуэль. Это психологический триллер о власти, морали, гордыне, справедливости и том, как быстро человек может превратиться в монстра, если решит, что имеет право судить всех.",

    "берсерк": "Гатс — одинокий воин с огромным мечом и тяжёлым прошлым. Его путь связан с наёмниками, войной, предательством, демонами и человеком по имени Гриффит, чья мечта о собственном королевстве меняет судьбы всех вокруг. Это тёмное фэнтези о боли, ярости, дружбе, травме, цене амбиций и попытке выжить в мире, где человек постоянно сталкивается с чудовищами — внешними и внутренними.",

    "киберпанк 2077": "Cyberpunk 2077 переносит в Найт-Сити — огромный город корпораций, банд, имплантов и неоновой грязи. Игрок становится V, наёмником, который после опасного задания получает в голову цифровой след легендарного рокера Джонни Сильверхенда. Теперь V ищет способ выжить, пока город предлагает сделки, предательства и шанс стать легендой. Это RPG о личности, цене бессмертия, свободе выбора и жизни в мире, где тело и память стали товаром.",

    "elden ring": "Elden Ring отправляет игрока в Междуземье — разрушенный фэнтези-мир, где полубоги, древние силы и проклятия оставили после себя руины, чудовищ и легенды. Игрок сам выбирает путь, исследует земли, сражается с боссами и постепенно собирает смысл мира через локации, предметы и встречи. Это игра для тех, кто любит мрачное фэнтези, сложные битвы, свободу исследования и ощущение, что каждая победа заслужена.",

    "baldurs gate 3": "Baldur's Gate 3 — ролевая игра во вселенной Dungeons & Dragons, где герой заражён паразитом иллитида и должен найти способ избавиться от угрозы, пока внутри него просыпается чужая сила. Игрок собирает спутников, делает выборы, которые меняют отношения и сюжет, сражается пошагово и решает конфликты разными способами. Это RPG про свободу решений, характеры персонажей, моральные последствия и настоящее приключение в фэнтези-мире.",
    ...
    "the last of us part i": "The Last of Us Part I рассказывает о Джоэле и Элли, которые путешествуют через разрушенную Америку после грибковой пандемии. Джоэл должен доставить девочку к группировке Цикад, потому что её иммунитет может быть ключом к спасению людей. Но дорога постепенно превращает задание в личную связь. Это эмоциональная игра о потере, доверии, жестокости выживания и выборе между долгом и любовью."
  }));

  function v230Description(item){
    const title = titleOfV230(item);
    const key = norm(title);
    if (REAL_SYNOPSIS.has(key)) return REAL_SYNOPSIS.get(key);

    const original = txt(item && (item.overview_ru || item.description_ru || item.overview || item.description || item.plot));
    const type = typeOfV230(item);
    const year = yearOfV230(item);
    const genres = genresOfV230(item).slice(0,4);
    const rating = ratingOfV230(item);
    const votes = votesOfV230(item);
    ...
    const vibe = arr(item && item.vibe).slice(0,4);

    let result = "";
    if (original && original.length > 80 && /[.!?…]$/.test(original)) {
      result += original;
    } else if (original && original.length > 40) {
      result += original.replace(/[,\s]+$/,"") + ".";
    }

    const parts = [];
    let intro = `${title || "Этот проект"} — ${type.toLowerCase()}${year ? ` ${year} года` : ""}`;
    if (genres.length) intro += ` в жанрах ${genres.join(", ")}`;
    intro += ".";
    parts.push(intro);

    if (type.toLowerCase().includes("игра")) {
      let p = "По ощущениям это проект для тех, кто выбирает не только по названию, но и по миру, механикам и атмосфере.";
      if (vibe.length) p += ` Главный вайб: ${vibe.join(", ")}.`;
      if (rel) p += ` Связь с другими источниками: ${rel}.`;
      if (universe) p += ` Вселенная: ${universe}.`;
      parts.push(p);
    } else if (type.toLowerCase().includes("аниме")) {
      parts.push("Здесь важны не только события сюжета, но и арки персонажей, эмоциональные пики, визуальный стиль и развитие конфликта от серии к серии.");
    } else if (type.toLowerCase().includes("сериал")) {
      parts.push("Сериал раскрывается постепенно: персонажи, конфликты и общая интрига становятся важнее с каждой серией, поэтому его стоит выбирать по атмосфере и теме.");
    } else if (type.toLowerCase().includes("книга") || type.toLowerCase().includes("манга") || type.toLowerCase().includes("комик") || type.toLowerCase().includes("раноб")) {
      parts.push("Это хорошая точка входа для тех, кто хочет понять первоисточник, лор и связь с экранизациями, играми или аниме.");
    } else {
      parts.push("Картина строится вокруг конфликта, атмосферы и героев, поэтому перед просмотром стоит ориентироваться на жанры, рейтинг и общее настроение истории.");
    }

    if (related.length) parts.push(`Похожие или связанные проекты: ${related.join(", ")}.`);
    if (rating) {
      let r = `Оценка около ${Number(rating).toFixed(1)}`;
      if (votes) r += ` при аудитории примерно ${votes >= 1000 ? Math.round(votes/1000) + " тыс." : votes + " голосов"}`;
      r += " помогает понять общий уровень интереса к проекту.";
      parts.push(r);
    }

    result = [result, ...parts].filter(Boolean).join(" ");
    return result;
  }

  displayOverview = function(item){
    return v230Description(item);
  };
  ...
  function patchModalText(){
    ...
    if (!title) return;
    const current = txt(overview.textContent);
    if (current.length > 260 && !/каталог|специально расширено|без просмотра трейлера/i.test(current)) return;

    const meta = txt(document.getElementById("detailMeta") && document.getElementById("detailMeta").textContent);
    const genres = txt(document.getElementById("detailGenres") && document.getElementById("detailGenres").textContent);
    const pseudo = {
      title,
      type: meta.split("·")[0] || "Фильм",
      year: (meta.match(/(19\d{2}|20\d{2})/) || [,""])[1],
      rating: (meta.match(/([0-9]+(?:\.[0-9]+)?)/) || [,""])[1],
      genres: genres ? genres.split("·").map(x=>x.trim()).filter(Boolean) : [],
      description: current.replace(/каталог[^.]+/ig,"").trim()
    };
    overview.textContent = v230Description(pseudo);
  }

  function injectStyle(){
    if (document.getElementById("gkm-v230-style")) return;
    const s = document.createElement("style");
    s.id = "gkm-v230-style";
    s.textContent = `
      #detailOverview{
  ...
  injectStyle();
  const obs = new MutationObserver(function(){ setTimeout(patchModalText, 50); });
  obs.observe(document.documentElement, {childList:true, subtree:true, characterData:true});
  document.addEventListener("click", function(){ setTimeout(patchModalText, 120); setTimeout(patchModalText, 500); }, true);
})();
 /* GKM V230 REAL SYNOPSIS FIX END */



/* GKM V231 REAL DESCRIPTION OVERRIDE START */
(function(){
  function txt(v){ return String(v == null ? "" : v).trim(); }
  function norm(v){
    return txt(v).toLowerCase()
  ...
  const SYNOPSIS = {
    ...
    "бойцовский клуб":
      "Главный герой страдает от бессонницы, пустоты и ощущения, что его жизнь превратилась в работу, покупки и бессмысленные правила. После встречи с Тайлером Дёрденом он создаёт подпольный бойцовский клуб, где мужчины выпускают злость и впервые чувствуют себя живыми. Но простые драки быстро превращаются в опасное движение. Это мрачная история о кризисе личности, потреблении, подавленной агрессии и свободе, которая выходит из-под контроля.",

    "матрица":
      "Хакер Нео узнаёт, что привычный мир — не реальность, а симуляция, созданная машинами для контроля над людьми. Морфеус и Тринити показывают ему настоящий мир, где человечество порабощено, а люди используются как источник энергии. Нео должен понять, кто он на самом деле, и принять роль в борьбе против системы. Это культовая фантастика о свободе выбора, пробуждении, контроле и границе между иллюзией и реальностью.",

    "темный рыцарь":
      "Готэм сталкивается с Джокером — преступником, который не ищет денег или власти, а хочет доказать, что любой порядок можно разрушить. Бэтмен, Гордон и Харви Дент пытаются остановить хаос, но Джокер заставляет их делать тяжёлый моральный выбор. Это супергеройский криминальный триллер о справедливости, страхе, символах и цене, которую платит герой, чтобы защитить город.",

    "атака титанов":
      "Люди живут за огромными стенами, спасаясь от титанов — гигантов, пожирающих людей. После разрушения стены Эрен Йегер теряет дом и клянётся уничтожить титанов. Вместе с Микасой и Армином он вступает в армию, но чем дальше идёт война, тем сложнее становится правда о мире. Это мрачное аниме о свободе, мести, войне, пропаганде и том, как ненависть меняет людей.",

    "наруто":
      "Наруто Узумаки — шумный подросток-ниндзя, которого деревня отвергала из-за запечатанного внутри него Девятихвостого лиса. Он мечтает стать Хокаге, чтобы добиться признания и доказать свою ценность. Вместе с Саске, Сакурой и Какаши он проходит миссии, тренировки и тяжёлые испытания. Это история о дружбе, одиночестве, силе воли, боли прошлого и желании быть принятым.",

    "ван пис":
      "Монки Д. Луффи отправляется в море, чтобы найти легендарное сокровище One Piece и стать Королём пиратов. По пути он собирает команду, где у каждого есть своя мечта и своя боль. Их путешествие превращается в огромную историю о свободе, дружбе, приключениях, жестоких режимах и тайнах мира. Это аниме для тех, кто любит большие вселенные, юмор, драму и долгий путь к мечте.",

    "тетрадь смерти":
      "Лайт Ягами находит тетрадь, которая убивает человека, если записать его имя. Решив очистить мир от преступников, он становится загадочным убийцей Кирой. Его противником становится гениальный детектив L, и между ними начинается опасная интеллектуальная дуэль. Это психологический триллер о власти, морали, гордыне и том, как человек может превратиться в монстра, если решит, что имеет право судить всех.",

    "берсерк":
      "Гатс — одинокий воин с огромным мечом и тяжёлым прошлым. Его путь связан с войной, наёмниками, демонами, предательством и Гриффитом — человеком, чья мечта о власти меняет судьбы всех вокруг. Это тёмное фэнтези о боли, ярости, дружбе, травме, амбициях и попытке выжить в мире, где чудовища бывают не только снаружи, но и внутри людей.",

    "cyberpunk 2077":
      "Cyberpunk 2077 переносит в Найт-Сити — город корпораций, банд, имплантов и неоновой грязи. Игрок становится V, наёмником, который после опасного задания получает в голову цифровой след легендарного Джонни Сильверхенда. Теперь V ищет способ выжить, пока город предлагает сделки, предательства и шанс стать легендой. Это RPG о личности, цене бессмертия, свободе выбора и мире, где тело и память стали товаром.",

    "elden ring":
      "Elden Ring отправляет игрока в Междуземье — разрушенный фэнтези-мир, где полубоги, древние силы и проклятия оставили после себя руины, чудовищ и легенды. Игрок сам выбирает путь, исследует земли, сражается с боссами и постепенно собирает смысл мира через локации, предметы и встречи. Это игра для тех, кто любит мрачное фэнтези, сложные битвы и чувство настоящего открытия.",

    "baldurs gate 3":
      "Baldur's Gate 3 — ролевая игра во вселенной Dungeons & Dragons, где герой заражён паразитом иллитида и должен найти способ избавиться от угрозы. Игрок собирает спутников, принимает решения, которые меняют сюжет и отношения, сражается пошагово и проходит ситуации разными способами. Это RPG про свободу выбора, последствия, характеры персонажей и настоящее фэнтези-приключение.",

    "the last of us part i":
      "The Last of Us Part I рассказывает о Джоэле и Элли, которые проходят через разрушенную Америку после грибковой пандемии. Джоэл должен доставить девочку к Цикадам, потому что её иммунитет может стать ключом к спасению людей. Но дорога постепенно превращает задание в личную связь. Это тяжёлая и эмоциональная игра о потере, доверии, жестокости выживания и выборе между долгом и любовью."
  };

  function currentModalTitle(){
    return txt(document.getElementById("detailTitle") && document.getElementById("detailTitle").textContent);
  }

  function currentGenres(){
    return txt(document.getElementById("detailGenres") && document.getElementById("detailGenres").textContent);
  }

  function currentMeta(){
    return txt(document.getElementById("detailMeta") && document.getElementById("detailMeta").textContent);
  }

  function genericSynopsis(title, meta, genres, oldText){
    const type = txt(meta.split("·")[0]) || "Проект";
    const year = (meta.match(/(19\d{2}|20\d{2})/) || [,""])[1];
    const genreList = genres ? genres.split("·").map(x=>x.trim()).filter(Boolean).slice(0,3) : [];
    let cleanOld = txt(oldText)
      .replace(/В каталоге.*$/i, "")
      .replace(/это описание специально расширено.*$/i, "")
      .replace(/чтобы можно было выбрать проект.*$/i, "")
      .trim();

    if (cleanOld.length > 80 && /[.!?…]$/.test(cleanOld)) return cleanOld;

    let text = `${title} — ${type.toLowerCase()}${year ? ` ${year} года` : ""}`;
    if (genreList.length) text += ` в жанрах ${genreList.join(", ")}`;
    text += ". ";

    if (cleanOld.length > 30) {
      text += cleanOld.replace(/[,\s]+$/,"") + ". ";
    }

    const g = genres.toLowerCase();
    if (g.includes("драма")) text += "В основе истории — конфликт персонажей, тяжёлый выбор и эмоциональное развитие. ";
    if (g.includes("криминал")) text += "Криминальная линия добавляет давление, риск, нарушение закона и столкновение человека с системой. ";
    if (g.includes("фантаст")) text += "Фантастическая часть строит мир с необычными правилами, технологиями или идеями. ";
    if (g.includes("фэнтези")) text += "Фэнтези делает акцент на мире, мифологии, силе героев и большом приключении. ";
    if (g.includes("ужас") || g.includes("хоррор")) text += "Хоррор держит на тревоге, опасности и ощущении, что герой может не выбраться. ";
    if (g.includes("боев") || g.includes("экшен")) text += "Экшен даёт динамику, столкновения и зрелищные события. ";
    if (g.includes("комед")) text += "Комедийная часть делает просмотр легче и добавляет юмор в развитие событий. ";
    if (g.includes("детектив") || g.includes("триллер")) text += "Детективно-триллерная часть держит интерес через тайну, расследование и постепенное раскрытие правды. ";

    text += "Такое описание должно помочь заранее понять сюжет, настроение и причину, почему проект стоит открыть дальше.";
    return text;
  }

  function getGoodText(title, meta, genres, oldText){
    const k = norm(title);
    if (SYNOPSIS[k]) return SYNOPSIS[k];

    // частичное совпадение для названий с годом/приписками
    for (const key in SYNOPSIS) {
      if (k.includes(key) || key.includes(k)) return SYNOPSIS[key];
    }

    return genericSynopsis(title, meta, genres, oldText);
  }
  ...
  function apply(){
    ...
    const old = txt(overview.textContent);
    const bad =
      old.length < 260 ||
      /каталог|специально расширено|без просмотра трейлера|заранее понять, чего ждать|подойдет тем, кто хочет/i.test(old);

    if (!bad) return;

    const next = getGoodText(title, currentMeta(), currentGenres(), old);
    if (next && next !== old) {
      overview.textContent = next;
      overview.dataset.gkmV231 = "1";
    }

    overview.style.whiteSpace = "normal";
    overview.style.lineHeight = "1.65";
    overview.style.fontSize = "17px";
    overview.style.fontWeight = "650";
  }

  function patchOpenDetails(){
    if (typeof openDetails !== "function" || window.GKM_V231_OPEN_PATCHED) return;
    ...
    window.GKM_V231_OPEN_PATCHED = "1";
  }
  ...
  const obs = new MutationObserver(function(){ setTimeout(apply, 40); });
  obs.observe(document.documentElement, {childList:true, subtree:true, characterData:true});
})();
 /* GKM V231 REAL DESCRIPTION OVERRIDE END */
