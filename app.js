const GKM_APP_CLEAN_VERSION = "v117-anime-top-100-people-rating-2026-06-23";
...
function makeSearchWorker() {
  ...
  const code = `
    ...
    function animeTopScore(x){const r=rating(x);const v=Math.max(1,votes(x));const m=180000;const c=7.35;const bayes=(v/(v+m))*r+(m/(v+m))*c;const pop=Math.log10(v)*0.10;const yearBonus=Math.min(Math.max(Number(year(x)||2000)-1990,0),35)*0.002;return bayes+pop+yearBonus+poster(x)*0.03;}
    function applyAnimeTopDedupe(){const cap=new Map();const out=[];for(const row of rows){const key=franchiseKey(row.item);const count=cap.get(key)||0;if(count>=2)continue;cap.set(key,count+1);out.push(row);if(out.length>=100)break;}rows=out;}
