...
function scheduleScan(records){
  const relevant=(records||[]).some(record=>{
    ...
    return !target?.closest?.('#gkmV379Integrated,.gkm-v379-status');
  });
  if(!relevant||scanFrame)return;
  scanFrame=requestAnimationFrame(()=>{scanFrame=0;scan()});
}
function init(){scan();new MutationObserver(scheduleScan).observe(document.body,{childList:true,subtree:true});window.GKM_MY_GOLUB={get:load,set:(i,s)=>{const d=load();d[keyOf(i)]={...i,status:s,updatedAt:Date.now()};save(d);renderIntegrated()}}}
