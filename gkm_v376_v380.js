...
function cardItem(card){const t=card.querySelector('.card-title,.title,h2,h3,[data-title]');const text=card.querySelector('.card-meta')?.textContent||'';const badge=card.querySelector('.card-badge')?.textContent||'';return{title:(t?.getAttribute('data-title')||t?.textContent||card.dataset.title||'').trim(),year:(card.dataset.year||((text.match(/\b(18\d{2}|19\d{2}|20\d{2})\b/)||[])[1])||'').trim(),type:(card.dataset.type||badge||((text.match(/Фильм|Сериал|Аниме|Мультфильм/i)||[])[0])||'').trim()}}

function addStatuses(card,i,stored){if(card.querySelector('.gkm-v379-status'))return;const bar=document.createElement('div');bar.className='gkm-v379-status';const k=keyOf(i),active=stored[k]?.status;STATUSES.forEach(([icon,name])=>{const b=document.createElement('button');b.type='button';b.textContent=icon;b.title=name;b.style.opacity=active===name?'1':'.62';b.onclick=e=>{e.preventDefault();e.stopPropagation();const x=load();x[k]={...i,status:name,updatedAt:Date.now()};save(x);bar.querySelectorAll('button').forEach(q=>q.style.opacity='.62');b.style.opacity='1';renderIntegrated()};bar.appendChild(b)});card.appendChild(bar)}
...
function renderIntegrated(){const host=findHost();if(!host)return;let box=host.querySelector('#gkmV379Integrated');if(!box){box=document.createElement('section');box.id='gkmV379Integrated';box.className='gkm-v379-integrated';host.appendChild(box)}const all=Object.values(load()).sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)),counts=Object.fromEntries(STATUSES.map(([,s])=>[s,all.filter(x=>x.status===s).length]));const signature=JSON.stringify(all.map(x=>[x.title,x.year,x.type,x.status,x.updatedAt]));if(box.dataset.gkmSignature===signature)return;box.dataset.gkmSignature=signature;box.innerHTML=`<div><b>🕊 Мой Голубь</b><span style="opacity:.7;font-size:12px"> — статусы и личная библиотека</span></div><div class="gkm-v379-stats">${STATUSES.map(([i,s])=>`<div class="gkm-v379-stat">${i}<br><b>${counts[s]||0}</b><br><span style="opacity:.7">${esc(s)}</span></div>`).join('')}</div><div class="gkm-v379-tabs">${STATUSES.map(([i,s])=>`<button type="button" data-status="${esc(s)}">${i} ${esc(s)} (${counts[s]||0})</button>`).join('')}</div><div class="gkm-v380-row"><button data-ai="посоветуй фильм на вечер">🍿 На вечер</button><button data-ai="лучшие популярные фильмы">🔥 Популярное</button><button data-ai="посоветуй лучшее аниме">🎌 Аниме</button><button data-ai="посоветуй страшный фильм">😱 Страшное</button><button data-ai="умный фильм с высоким рейтингом">🧠 Умное</button></div><div class="gkm-v379-list" id="gkmV379List"></div>`;const list=box.querySelector('#gkmV379List');const show=s=>{const rows=s?all.filter(x=>x.status===s):all;list.innerHTML=rows.length?rows.slice(0,100).map(x=>`<div class="gkm-v379-row"><div><b>${esc(x.title||'Без названия')}</b><div style="opacity:.65;font-size:12px">${esc([x.year,x.type].filter(Boolean).join(' • '))}</div></div><span>${esc(x.status||'')}</span></div>`).join(''):'<div style="opacity:.65;padding:10px 0">Пока пусто.</div>'};show();box.querySelectorAll('[data-status]').forEach(b=>b.onclick=()=>show(b.dataset.status));box.querySelectorAll('[data-ai]').forEach(b=>b.onclick=()=>runAi(b.dataset.ai))}

function hook(){[...document.querySelectorAll('button,a,[role="button"]')].filter(x=>/(?:мой\s+список|мои\s+списки)/i.test((x.textContent||'').trim())).forEach(b=>{if(b.dataset.gkmV2)return;b.dataset.gkmV2='1';b.addEventListener('click',()=>setTimeout(renderIntegrated,150))})}
function scan(cards){addStyle();const stored=load();cards.forEach(c=>{if(!c.querySelector('.gkm-v379-status'))addStatuses(c,cardItem(c),stored)});hook();if(document.getElementById('gkmV379Integrated'))renderIntegrated()}
let scanFrame=0;
const pendingCards=new Set();
function scheduleScan(records){
  for(const record of records||[]){
    for(const node of record.addedNodes){
      if(node.nodeType!==1||node.closest?.('#gkmV379Integrated,.gkm-v379-status'))continue;
      if(node.matches?.('.card'))pendingCards.add(node);
      node.querySelectorAll?.('.card').forEach(card=>pendingCards.add(card));
    }
  }
  if(!pendingCards.size||scanFrame)return;
  scanFrame=requestAnimationFrame(()=>{scanFrame=0;const cards=[...pendingCards].filter(card=>card.isConnected);pendingCards.clear();if(cards.length)scan(cards)});
}
function init(){scan(document.querySelectorAll('.card'));new MutationObserver(scheduleScan).observe(document.body,{childList:true,subtree:true});window.GKM_MY_GOLUB={get:load,set:(i,s)=>{const d=load();d[keyOf(i)]={...i,status:s,updatedAt:Date.now()};save(d);renderIntegrated()}}}
