[GKM_PROJECT_SCHEME_V1.md](https://github.com/user-attachments/files/29473708/GKM_PROJECT_SCHEME_V1.md)
...
## GKM V202 GAME HUB FULL FEATURES

Раздел `🎮 Игры` теперь считается отдельным игровым хабом, а не просто списком карточек.
...
🎮 Игры
 ...
 │   ├─ Общая вселенная
 │   └─ Кино-вайб
 ├─ карточки игровых вселенных
 │   ├─ вертикальный постер
 │   ├─ тип связи
 │   └─ мини-бейджи
 └─ модалка игры
     ├─ Где купить / где играть
     ├─ Связанные фильмы / сериалы / аниме
     ├─ Во что играть после просмотра
     ├─ Гайды и трейлеры
     ├─ Хронология вселенной
     ├─ Похожие игры
     ├─ Похожие фильмы / сериалы по вайбу
     └─ Вайб
```

Важное правило: V202 не смешивает игровые данные с основной базой фильмов. Игры живут отдельно в `data/games_catalog.json`.

Если раздел не понравится, откат простой:

```txt
1. вернуть старый app.js
2. вернуть старый index.html при необходимости
3. удалить data/games_catalog.json
```
