[Uploading GKM_PROJECT_SCHEME_V1_V263_UPDATED.txt…]()
...
## GKM V228 HARD POSTERS + FULL DESCRIPTIONS FIX
- жёсткий фикс постеров: если у игры или любого проекта нет картинки, сайт генерирует карточке полноценный SVG-постер по названию и типу
...
## GKM V230 REAL SYNOPSIS FIX
- убраны шаблонные описания про каталог
- добавлены нормальные синопсисы для популярных фильмов, аниме и игр
- общий генератор описаний теперь пишет о сюжете, атмосфере, жанре и связанных проектах, а не о том, что сайт является каталогом

## GKM V231 REAL DESCRIPTION OVERRIDE
...
## GKM V261 STRICT ANIME TOP CLEAN FIX

Дата: 2026-07-01
Рабочая зона: `app.js`, `data/fast/anime_top_manual.json`

### Что исправлено

На первой странице `Топ аниме 100` попадали карточки типа `Мультфильм` и проекты с маленькими голосами: 10 тыс, 18 тыс, 433 тыс.

Причина: топ строился слишком широко и мог брать не чистые anime-записи.
...
Топ аниме 100
 ├─ только type = Аниме
 ├─ источник/жанр должен подтверждать anime / Jikan / MyAnimeList
 ├─ Disney / Pixar / Marvel / Мультфильм не проходят
 ├─ сортировка строго по votes сверху вниз
 ├─ мусор с 10к / 18к / 433к голосов не проходит
 └─ текущая база MAL имеет максимум около 3 млн голосов, поэтому 4 млн в этой базе физически нет
```

### Важное уточнение

Если в будущем база даст аниме с 4 000 000+ голосов, они автоматически будут наверху. Сейчас максимум в `search_lite.json` около 3 077 019 голосов, поэтому топ начинается с максимума базы, а не с мультфильмов/мусора.
...
## GKM V262 CARTOONS NO ANIME FIX

Дата: 2026-07-01  
Рабочая зона: `app.js`, раздел `Мультфильмы`

### Что было сломано

В разделе `Мультфильмы` на первой и следующих страницах попадались аниме-тайтлы и японские anime movies, потому что часть таких проектов в базе имела `type = "Мультфильм"`.

Примеры нежелательных карточек в разделе мультфильмов:
...
Раздел `Мультфильмы` теперь проходит через строгий фильтр `isPureCartoon()`:

```text
Мультфильмы
 ├─ type должен быть "Мультфильм"
 ├─ аниме-источники Jikan / MyAnimeList запрещены
 ├─ жанр "аниме" запрещён
 ├─ anime-слова и студии Ghibli / MAPPA / Pierrot / Madhouse / ufotable / Toei запрещены
 ├─ известные anime movies выкидываются из раздела
 └─ выдача дальше сортируется обычной умной сортировкой по голосам / рейтингу / постеру
```

### Важно

Это не фикс `Топ аниме 100`.  
Это отдельный фикс обычного раздела `Мультфильмы`.

Если в будущем в мультфильмах снова появятся аниме-тайтлы, смотреть первым делом:

```text
worker-функции:
isAnimeLikeAny()
isPureCartoon()
tabPass() -> if(tab==="cartoons")return isPureCartoon(x);
```
...
## GKM V263 UNIVERSAL DETAIL DESCRIPTION ALL SECTIONS

Дата: 2026-07-01  
Рабочая зона: `app.js`, модалка подробностей, `detailOverview`

### Проблема

В карточках разных разделов появлялся шаблонный текст вида:

```text
Карточка будет дополнена подробным описанием позже.
```

Пример: `Гравити Фолз` в разделе `Мультфильмы` показывал техническое описание каталога, а не нормальное описание проекта.

### Что исправлено

- Описание теперь принудительно работает для всех главных разделов:
  - фильмы;
  - сериалы;
  - мультфильмы;
  - аниме;
  - игры;
  - книги / манга / комиксы.
- Старые шаблоны `из каталога` и `будет дополнена позже` считаются плохим описанием и заменяются.
- Если у популярного проекта есть ручной синопсис, он ставится первым.
- Если реального описания нет, создаётся нормальное универсальное описание по типу, году, жанрам, рейтингу и голосам.
- После открытия модалки текст обновляется несколько раз, чтобы старые V223/V259-наблюдатели не перетёрли описание обратно.

### Главное правило

```text
В модалке не должно быть технических заглушек.
Любой раздел должен показывать нормальное человеческое описание.
```

### Проверка после патча

```text
1. Открыть Фильмы → карточка → есть описание.
2. Открыть Сериалы → карточка → есть описание.
3. Открыть Мультфильмы → Гравити Фолз → нет фразы "будет дополнена позже".
4. Открыть Аниме → карточка → есть описание.
5. Открыть Игры → карточка → есть описание.
6. Открыть Книги/Манга → карточка → есть описание.
```
