...
class OfficialRussianResolver:
    ...
    def __init__(self, root: Path, stats: Counter, online: bool = True):
        ...
        self.stats = stats
        self.online = online
        self.cache_path = root / "data" / "ru_official_cache_v3451.json"
        self.cache = {"version": "V3451", "items": {}}
    ...
    def _needs_title(self, item: dict) -> bool:
        return not has_cyr(self._current_title(item))
    ...
    def _prefetch_shikimori(self, items: list[dict]):
        ...
        for item in items:
            ...
            if match and self._needs_title(item) and iid not in self.cache["items"]:
                ids.append(match.group(1))
        ids = sorted(set(ids), key=int)
        self.stats["shikimori_candidates"] = len(ids)

        # Shikimori anime IDs correspond to MAL IDs in this catalog.
        for offset in range(0, len(ids), 50):
            ...
            # Fallback for IDs not returned by the batch endpoint.
            missing = [x for x in batch if x not in returned]
            for mal_id in missing:
                ...
                if payload:
                    payload["source"] = "Shikimori"
                    payload["original"] = str(row.get("name") or "").strip()
                    self._store(f"mal_{mal_id}", payload)
    ...
    def _tmdb_one(self, item: dict):
        ...
        if self.tmdb_key:
            ...
            return iid, None
        url = f"https://api.themoviedb.org/3/{endpoint}/{iid}?{urllib.parse.urlencode(params)}"
        ...
        if payload:
            payload["source"] = "TMDB ru-RU"
        return iid, payload or None
    ...
    def _prefetch_tmdb(self, items: list[dict]):
        ...
        candidates = [
            ...
            if self._needs_title(item)
            and self._is_tmdb_item(item)
            and str(item.get("id")) not in self.cache["items"]
        ]
    ...
    def _kp_one(self, item: dict):
        iid = str(item.get("id") or "")
        row = self._get_json(
            f"https://api.kinopoisk.dev/v1.4/movie/{iid}",
    ...
    def _prefetch_kinopoisk(self, items: list[dict]):
        ...
        for item in items:
            ...
            if (
                iid.isdigit()
                and "kinopoisk" in source
                and self._needs_title(item)
                and iid not in self.cache["items"]
            ):
...
def process_poster_wall(root: Path,items_by_id: dict[str,dict],stats: Counter,dry=False):
    ...
    if manifest_path.exists():
        try:
            ...
            manifest={}
        manifest['version']='v3451-official-russian-dedupe'
...
def main():
    ...
    unresolved=[
        ...
        for x in items
        if x.get('titleLocalizationSource')=='transliteration'
    ]
    if not args.dry_run:
        (root/'data/unresolved_official_ru_v3451.json').write_text(
    ...
    (root/'TEST_REPORT_V3451_OFFICIAL_RU_DEDUPE.json').write_text(
