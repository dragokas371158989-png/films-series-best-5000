...
class OfficialRussianResolver:
    ...
    def __init__(self, root: Path, stats: Counter, online: bool = True):
        ...
        self.stats = stats
        self.online = online
        self.cache_path = root / "data" / "ru_official_cache_v3452_1.json"
        self.cache = {"version": "V3452.1", "items": {}}
        if self.cache_path.exists():
            ...
            previous = root / "data" / "ru_official_cache_v3452.json"
...
def canonical_id_priority(x: dict) -> int:
    """Keep stable source IDs when cross-source duplicates are merged."""
    iid=str(x.get('id') or '').strip()
    typ=localize_type(x.get('type') or x.get('category'))
    source=norm(x.get('source'))
    if typ in ('Аниме','Мультсериал') and re.fullmatch(r'mal_\d+',iid):
        return 1000
    if re.fullmatch(r'kp_\d+',iid):
        return 900
    if re.fullmatch(r'tmdb_\d+',iid):
        return 800
    if iid.isdigit() and 'tmdb' in source:
        return 700
    if iid.isdigit():
        return 600
    if iid:
...
def canonical_choice_key(x: dict) -> tuple:
    return (canonical_id_priority(x), quality(x))


def merged_ids_of(x: dict) -> set[str]:
    values=set()
    iid=str(x.get('id') or '').strip()
    if iid:
        values.add(iid)
    for value in x.get('mergedDuplicateIds') or []:
...
def build_canonical_lookup(items: list[dict]) -> dict[str,dict]:
    lookup={}
    for item in items:
        for iid in merged_ids_of(item):
            lookup[iid]=item
    return lookup
...
def merge_items(items: list[dict], stats: Counter) -> list[dict]:
    ...
    for iid,group in by_id.items():
        if len(group)==1:
            item=group[0].copy()
            item['mergedDuplicateIds']=sorted(merged_ids_of(item)-{iid})
            id_clean.append(item)
            continue
        best=max(group,key=canonical_choice_key).copy()
        aliases=[]
        merged=set()
        for x in group:
            aliases.extend(x.get('aliases') or [])
            aliases.extend(str(x.get(k) or '').strip() for k in ('ru','en','title','name','originalTitle') if x.get(k))
            merged.update(merged_ids_of(x))
            for k,v in x.items():
                if (k not in best or best.get(k) in ('',None,[],{})) and v not in ('',None,[],{}):
                    best[k]=v
        canonical=str(best.get('id') or '').strip()
        merged.discard(canonical)
        seen=set()
        best['aliases']=[a for a in aliases if a and not (norm(a) in seen or seen.add(norm(a)))]
        best['mergedDuplicateIds']=sorted(merged)
        stats['duplicates_removed']+=len(group)-1
        id_clean.append(best)

    # 2) Cross-source duplicates by localized title + year + type, but only
    # when original title or poster proves they are the same work.
    groups=defaultdict(list)
    for x in id_clean+without_id:
        groups[strong_dup_key(x)].append(x)
    ...
    for key,group in groups.items():
        ...
        for x in group:
            ...
            for cluster in clusters:
                if any(evidence_overlap(x,y) for y in cluster):
                    cluster.append(x)
                    placed=True
                    break
            if not placed:
        ...
        for cluster in clusters:
            if len(cluster)==1:
                ...
                continue
            best=max(cluster,key=canonical_choice_key).copy()
            aliases=[]
            merged=set()
            for x in cluster:
                aliases.extend(x.get('aliases') or [])
            ...
            best['aliases']=[a for a in aliases if a and not (norm(a) in seen or seen.add(norm(a)))]
            best['mergedDuplicateIds']=sorted(merged)
            stats['duplicates_removed']+=len(cluster)-1
            stats['cross_source_duplicates_removed']+=len(cluster)-1
            out.append(best)
    return out
...
def rebuild_search(root: Path,items: list[dict],stats: Counter,dry=False):
    ...
    for x in items:
        ...
        row['mergedDuplicateIds']=x.get('mergedDuplicateIds') or []
...
def process_poster_wall(root: Path,items_by_id: dict[str,dict],stats: Counter,dry=False):
    ...
    for prefix in ('movies','series','anime','cartoons'):
        ...
        for p in files:
            ...
            for row in data:
                ...
                canonical_id=str(item.get('id') or iid) if item else iid
                dedupe_key=(prefix,canonical_id) if canonical_id else (prefix,norm(row[1]),str(row[3] if len(row)>3 else ''))
    ...
    if seed_path.exists():
        ...
        for row in seed if isinstance(seed,list) else []:
            ...
            item=items_by_id.get(iid)
            canonical_id=str(item.get('id') or iid) if item else iid
            key=canonical_id or (norm(row[1]),str(row[3] if len(row)>3 else ''))
            if key in seen:
                stats['poster_seed_duplicates_removed']+=1
                continue
            seen.add(key)
            if item:
                row[0]=canonical_id
    ...
    if manifest_path.exists():
        try:
            ...
            manifest={}
        manifest['version']='v3452.1-official-russian-repair'
        ...
        def walk(v):
            ...
            if isinstance(v,dict):
                ...
                if iid and iid in items_by_id:
                    src=items_by_id[iid]
                    v['id']=str(src.get('id') or iid)
                    for k in ('ru','en','year','type','rating','votes','poster','genres','overview','status','aliases','mergedDuplicateIds'):
                        if k in src:
...
def main():
    ...
    byid=build_canonical_lookup(items)
    ...
    stats['merged_duplicate_alias_ids']=sum(len(x.get('mergedDuplicateIds') or []) for x in items)
    ...
    if not args.dry_run:
        (root/'data/unresolved_official_ru_v3452_1.json').write_text(
    ...
    (root/'TEST_REPORT_V3452_1_OFFICIAL_RU_REPAIR.json').write_text(
