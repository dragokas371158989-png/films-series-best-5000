...
REPORT = ROOT / "TEST_REPORT_V3453_FULL_RUSSIAN.json"
OFFICIAL_CACHE_OUT = ROOT / "data" / "ru_complete_cache_v3453.json"
MACHINE_CACHE_PATH = ROOT / "data" / "ru_machine_cache_v3453.json"
...
def localize_genres(value: Any) -> list[str]:
    ...
    for part in parts:
        ...
        if not part:
            continue
        localized = GENRE_MAP.get(norm(part), part)
        key = norm(localized)
...
def apply_localization(item: dict, official: dict[str, dict], translator: RussianTranslator, stats: Counter) -> dict:
    ...
    raw_type = norm(result.get("type") or result.get("category"))
    localized_type = TYPE_MAP.get(raw_type, result.get("type") or result.get("category") or "Проект")
    result["type"] = localized_type
    if "category" in result:
        result["category"] = localized_type

    result["genres"] = localize_genres(result.get("genres"))

    if "status" in result:
        result["status"] = STATUS_MAP.get(norm(result.get("status")), result.get("status"))
...
def update_wall_base(base: Path, by_id: dict[str, dict], stats: Counter):
    ...
    if isinstance(manifest, dict):
        manifest["version"] = "v3453-full-russian"
...
def validate(items: list[dict], stats: Counter):
    ...
    for item in items:
        ...
        for genre in localize_genres(item.get("genres")):
            if has_lat(genre) and not has_cyr(genre):
