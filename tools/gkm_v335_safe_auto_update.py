#!/usr/bin/env python3
"""GKM V335 — safe append-only Kinopoisk catalog update.
...
VERSION="v335-safe-auto-update-2026-07-12"
DATA=Path("data")
TMP=DATA/".gkm_v335_tmp"
REPORT=DATA/"gkm_v335_auto_update_report.json"
...
def map_doc(doc,typ):
    ...
    item={"id":f"kp_{kid}","kinopoiskId":kid,"ru":ru,"en":en,"year":doc.get("year") or "","type":typ,"category":typ,
          ...
          "source":"kinopoisk_auto_v335","updated_at":now()}
    return {k:v for k,v in item.items() if v not in (None,"",[],{})}
def request(params):
    url=API+"?"+urlencode(params,doseq=True)
    req=Request(url,headers={"accept":"application/json","X-API-KEY":KEY,"User-Agent":"GKM-Safe-AutoUpdate/335"})
    try:
