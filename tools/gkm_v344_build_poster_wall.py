#!/usr/bin/env python3
"""V344: rebuild compact poster_wall_v333 data with strict kind+id dedupe."""
import json,re,shutil
from pathlib import Path
from datetime import datetime,timezone
SOURCES=[Path("data/fast/search_index.json"),Path("data/fast/search_lite.json")]
...
def main():
    if SRC is None:
        tried=", ".join(str(p) for p in SOURCES)
        raise SystemExit(f"poster wall source missing; tried: {tried}")
    data=json.loads(SRC.read_text(encoding="utf-8")); pools={k:[] for k in ("movies","series","anime","cartoons")};seen=set()
    for it in data:
        name,code=kind(it)
        item_id=clean(it.get("id") or it.get("kinopoiskId") or it.get("tmdbId") or it.get("mal_id"))
        if item_id:
            k=f"{name}|id:{item_id}"
        else:
            k=f"{name}|title:{clean(it.get('ru') or it.get('title') or it.get('name')).lower()}|year:{it.get('year') or ''}"
        if k in seen:continue
        seen.add(k);pools[name].append(compact(it,code))
    if TMP.exists():shutil.rmtree(TMP)
    TMP.mkdir(parents=True)
    unique_total=sum(map(len,pools.values()))
    manifest={"version":"v344-strict-kind-id-dedupe-poster-wall-2026-07-12","source":str(SRC),"generatedAt":now(),"sourceTotal":len(data),"total":unique_total,"duplicatesRemoved":max(0,len(data)-unique_total),"chunkSize":CHUNK,"seed":"seed_all.json","types":list(pools),"kinds":{}}
